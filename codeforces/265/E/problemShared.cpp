...
public:
  ComposeNumber(shared_ptr<Number> a, shared_ptr<Number> b,
                NumbersFactory &factory)
      : left_(a), right_(b),
        hash_(composeHash(a->hash(), b->hash(), b->bitLength())),
        length_(a->bitLength() + b->bitLength()), factory_(factory) {
    ++CC;
    assert(CC < 10000000);
    MC = max(MC, CC);
  }
  size_t bitLength() const override { return length_; }
  ...
  void setBits(size_t bitsBegin, size_t bitsEnd, bool value) override {
    ...
    if (bitsEnd >= length_ / 2) {
      setBits(right_, max(length_ / 2, bitsBegin) - length_ / 2,
              bitsEnd - length_ / 2, value);
    }
...
private:
  void setBits(shared_ptr<Number> &x, size_t bitsBegin, size_t bitsEnd,
               bool value);
...
public:
  std::shared_ptr<Number> get(bool value, size_t len) {
    ...
    return data_[key] = make_shared<ComposeNumber>(get(value, len / 2),
...
void ComposeNumber::setBits(shared_ptr<Number> &x, size_t bitsBegin,
                            size_t bitsEnd, bool value) {
