...
struct Answer {
  ...
  static const int kElementsPerMessage = 800 * 1000;
  void sent_to(int node) const {
    int messages =
        (data.size() + kElementsPerMessage - 1) / kElementsPerMessage;
    PutInt(node, messages);
    Send(node);
    for (int i = 0; i < data.size(); i += kElementsPerMessage) {
      int len = min(kElementsPerMessage, (int)data.size() - i);
      PutInt(node, len);
      for (int j = 0; j < len; ++j) {
        PutLL(node, data[i + j]);
      }
  ...
  static Answer receive_from(int node) {
    Answer m;
    Receive(node);
    int messages = GetInt(node);
    for (int i = 0; i < messages; ++i) {
      Receive(node);
      int len = GetInt(node);
      for (int j = 0; j < len; ++j) {
        m.data.push_back(GetLL(node));
      }
    }
    return m;
  }
...
int main() {
  ...
  if (is_master()) {
    ...
    vector<ll> data;
    if (my_node_id * K + K - 1 < number_of_nodes) {
      Task task1, task2;
