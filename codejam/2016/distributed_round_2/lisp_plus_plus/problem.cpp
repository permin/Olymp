...
int main() {
  ...
  if (is_master()) {
    vector<int> deltas(number_of_nodes);
    vector<int> min_vals(number_of_nodes);
    deltas[0] = delta;
    min_vals[0] = min_value;
    for (int node = 1; node < number_of_nodes; ++node) {
      Receive(node);
      deltas[node] = GetInt(node);
      min_vals[node] = GetInt(node);
    }
    int d = 0;
    int fail_seg = -1;
    ...
    PutInt(master_node(), delta);
    PutInt(master_node(), min_value);
    Send(master_node());
  }
