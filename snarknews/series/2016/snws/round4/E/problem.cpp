...
int maxFlow(vector<vector<Edge>> &al, int source, int sink) {
  ...
  while (true) {
    ...
    while (cur != source) {
      ...
      assert(e.cap >= 0);
      e.flow += add;
      al[v2][e.backEdge].cap += add;
      al[v2][e.backEdge].flow -= add;
...
vii doit(vector<vector<Edge>> net, int source, int sink, int n, int m,
  ...
  if (maxFlow(net, source, sink) != total) {
    return am;
  }
...
int main() {
  ...
  ans.push_back(doit(net, source, sink, n, m, total));
  double t2 = static_cast<double>(clock());
  double dt = 1.0 * (t2 - t1) / CLOCKS_PER_SEC;
  debug(dt);
  for (int i = 2; i < 20; ++i) {
    ans.push_back(doit(net, source, sink, n, m, total));
    dt = static_cast<double>(clock()) / CLOCKS_PER_SEC / i;
    if (static_cast<double>(clock()) / CLOCKS_PER_SEC + dt > 1.0) {
      break;
    }
  }
