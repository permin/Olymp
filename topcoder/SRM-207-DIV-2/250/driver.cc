...
int	time;
		...
		__in >> __expected >> speed >> positions >> velocities >> time;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << speed << ", " << positions << ", " << velocities << ", " << time << std::endl;
		std::cout.flush();

		__timer_start();

		TransportCounting __object;
		int __actual = __object.countBuses(speed, positions, velocities, time);
