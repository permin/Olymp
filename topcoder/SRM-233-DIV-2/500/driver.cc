...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		double	__expected;
		vector <int>	weldLocations;
int	L;

		__in >> __expected >> weldLocations >> L;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << weldLocations << ", " << L << std::endl;
		std::cout.flush();

		__timer_start();

		PipeCuts __object;
		double __actual = __object.probability(weldLocations, L);
