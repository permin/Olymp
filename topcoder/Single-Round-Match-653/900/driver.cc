...
vector <int>	card;
		...
		__in >> __expected >> win >> lose >> tie >> card;
		if (!__in.good()) break;

		std::cout << "===============Test " << __testnum<< "==================\n";
		
		std::cout << "input for test: " << win << ", " << lose << ", " << tie << ", " << card << std::endl;
		std::cout.flush();

		__timer_start();

		RockPaperScissorsMagic __object;
		int __actual = __object.count(win, lose, tie, card);
